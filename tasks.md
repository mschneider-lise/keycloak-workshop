...
Viel mehr passiert in den ersten Aufgaben auch gar nicht.
...
* Öffne die [Swagger-UI](http://localhost:8080/swagger) und rufe den "/public" Endpunkt auf.
  * Es sollte bestenfalls kein Fehler auftreten.
...
* Erstelle einen neuen Client im Realm, welcher später vom Frontend verwendet werden soll.
    ...
    * Achte darauf, dass du die Redirect URI korrekt setzt und denk auch an CORS.
...
* Nutze den `AuthProvider` der `oidc-react` library, um die `SecuredApp` Komponente vor unbefugtem Zugriff abzusichern.
...
Eine Möglichkeit, alle Realms inklusive Nutzer zu exportieren/importieren darfst du jetzt einmal ausprobieren.
...
* Ein weiteres Terminal im Pfad öffnen, in welchem die Exportdatei liegen soll.
  * Die Exportdatei aus dem Debug Container kopieren mit `docker cp keycloak-workshop-keycloak-debug:/tmp/export/realm.json .`.
* Die Shell im Debug Container mit `exit` schließen.


* Lösche die Keycloak Instanz über `docker compose down` und lege sie danach wieder mittels `docker compose up` an.
...
TODO
* Hinweis: In der `realm.json` Datei im Git-Repo ist der Nutzer `test` mit Passwort `Test` enthalten.
  * Dies ist relevant, wenn du ein Git Tag auscheckst und die Datei von Keycloak beim Start importiert wird.
...
* Füge deinem Client den neuen Scope hinzu und verifiziere, ob die Property im JWT enthalten ist.
  ...
  * Ist der Wert nicht im JWT enthalten, ist der Scope vielleicht nicht als "Default" definiert und ist daher nicht Teil des JWT.Als Lösung kann der Scope entweder in Keycloak als Default markiert werden oder der Scope, der im `AuthProvider` gesetzt ist, manuell angepasst werden.
